	...
	private static function locate_template($fileNameSearch, $filePathArray){
		...
		foreach($fileNameSearch as $fileName) {
			if('index.php' === $fileName) {
				...
				$plugin_path = implode('/', array_slice($filePathArray, 1)).'/';
			} else {
				$theme_path  = implode('/', array_slice($filePathArray, 0, count($filePathArray)-0)).'/';
				$plugin_path = implode('/', array_slice($filePathArray, 1, count($filePathArray)-2)).'/';
			}
